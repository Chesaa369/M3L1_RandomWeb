...
secret_list = ["Wow you found the secret thing eh? Nice!", "Head", "Tail"]

@app.route("/")
def home():
    return f'<h1>Welcome! To my website. Click the link down below if want to see some fun random facts</h1> <a href="/facts">View a random fact!</a>'
...
@app.route("/secret")
def secret():
    return f'<h1>{random.choice(secret_list)}</h1>'
